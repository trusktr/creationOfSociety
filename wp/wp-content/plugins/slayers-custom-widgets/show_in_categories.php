			...
			echo '<form method="post" action="', $url ,'"><div class="tablenav">';
				...
				echo '<div class=" alignright">';
					...
					echo '<select name="pagen">';
						for($i = 0; $i < $count / $this->info['posts_per_page']; ++$i)
							echo '<option ', $_GET['cat_pag'] == $i ? 'selected="selected"' : false ,' value="', $i ,'">', $i+1 ,'</option>';
					echo '</select>';
				...
				echo '<div class="align_left tablenav-pages" style="float:left">';
					...
					if( $_GET['cat_pag']+1 < ($count / $this->info['posts_per_page']) )
						echo '<a class="page-numbers" href="', str_replace( '&amp;cat_pag='.$_GET['cat_pag'],'&amp;cat_pag='.($_GET['cat_pag']+1),$url) ,'">&laquo; Next</a>';
					
					if($_GET['cat_pag'])
						echo '<a class="page-numbers" href="', str_replace('&amp;cat_pag='.$_GET['cat_pag'],'&amp;cat_pag='.($_GET['cat_pag']-1),$url) ,'">Previous &raquo;</a>';
