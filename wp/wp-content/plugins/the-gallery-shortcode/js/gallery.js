jQuery(document).ready(function($) {
	...
	$('div.media-item').each(function(i){
		var thiz = $(this),
			id = thiz.attr('id').match(/\d+$/);
		thiz.find('div.filename span').before( '<input class="exclude-check" id="exclude-check-'+ id +'" type="checkbox" data-id="'+ id +'" /> &nbsp;&nbsp;' );
	});
...
wpgallery = {
	...
	setup : function() {
		...
		if ( t.el.nodeName != 'IMG' || ! ed.dom.hasClass(t.el, 'wpGallery') ) {
			if ( (g = ed.dom.select('img.wpGallery')) && g[0] ) {
				...
				if ( getUserSetting('galperpage') ) t.I('itemsperpage').value = getUserSetting('galperpage');
				if ( getUserSetting('galexcludes') ) {
					jQuery.each(getUserSetting('galexcludes'), function(i, id){
						jQuery( '#exclude-check-'+id )[0].checked = true;
					})
				}
		...
		if ( a ) {
			...
			items_per_page = a.match(/items_per_page=['"]([^'"]+)['"]/i);
			var exclude = a.match(/exclude=['"]([^'"]+)['"]/i);

			if( link && link[1] ) t.I('linkto').value = link[1];
			...
			if( items_per_page && items_per_page[1] ) t.I('itemsperpage').value = items_per_page[1];
	...
	update : function() {
		...
		all = all.replace(/\s*(order|link|columns|orderby|items_per_page|exclude)=['"]([^'"]+)['"]/gi, '');
		all += t.getSettings();
	...
	getSettings : function() {
		var I = this.I, s = '';
		var $excludes = jQuery('input.exclude-check').filter(':checked'),
			excludes = [];

		if ( I('linkto').value != 'file' ) {
			s += ' link="'+ I('linkto').value +'"';
			setUserSetting('linkto', I('linkto').value );
		}
		...
		if ( I('itemsperpage').value != '0' ) {
			s += ' items_per_page="'+I('itemsperpage').value+'"';
			setUserSetting('galperpage', I('itemsperpage').value);
		}

		if( $excludes.length > 0 ) {
			$excludes.each(function(){
				excludes.push( jQuery(this).data('id') );
			});
			s += ' exclude="' + excludes.join(',') + '"';
			setUserSetting( 'galexcludes', excludes );
		}
